...
export async function runPerplExecutionRehearsal(options: RehearsalOptions = {}) {
  ...
  const transport: PerplExecutionTransport = {
    request: async (intent) => {
      calls.push(intent);
      const event = intent.action === "place"
      ...
      return event === "confirmed"
        ...
        : { version: 1, id: intent.id, event, actionId: intent.actionId, reason: `${event} rehearsal outcome` };
    },
  };
  const journalPath = options.journalPath ?? join(mkdtempSync(join(tmpdir(), "perpl-execution-rehearsal-")), "journal.json");
  const controller = new PerplMainnetCanaryController(new PerplCanaryExecutor(transport), {
    market: "BTCUSD", journalPath, now: () => 1_000,
  });
...
export function rehearsalPlan(): DryRunPlan {
  return {
    market: "BTCUSD", generatedAt: 1_000,
    reconciliation: { market: "BTCUSD", healthy: true, openOrderCount: 0, anomalies: [], checkedAt: 1_000 },
    positionBaseSize: 0, markPrice: 77_000, observedOpenOrders: [],
    balances: [{ token: "AUSD", amount: 18 }], accountEvidence: { frozen: false },
    positionSafetyEvidence: { baseSize: 0, markPrice: 77_000, deposit: 0, maintenanceRequirement: 0, liquidationPrice: 0, bankruptcyPrice: 0 },
    proposedCancellations: [],
    proposals: [{ side: "buy", price: 77_000, size: 0.00018, type: "postOnly", reduceOnly: false, allowed: true }],
    executionReady: false, readinessBlockers: ["offline rehearsal only"],
  };
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  runPerplExecutionRehearsal().then((result) => console.log(JSON.stringify(result, null, 2))).catch((error) => {
