...
async function confirm(phrase: string): Promise<void> {
  if (!process.stdin.isTTY)
    throw new Error("RISEx Live requires a human at an interactive terminal");
  const rl = createInterface({ input: process.stdin, output: process.stdout });
  try {
    if ((await rl.question(`\nType exactly: ${phrase}\n> `)) !== phrase)
      throw new Error("confirmation phrase did not match; RISEx Live NOT started");
  } finally {
    rl.close();
  }
}
...
async function main(): Promise<void> {
  ...
  if (!account || !/^0x[0-9a-fA-F]{40}$/.test(account))
    throw new Error("RISEX_ACCOUNT_ADDRESS is missing or invalid");
  ...
  const config = loadMarketsConfig(
    process.env.RISEX_MARKETS_CONFIG_PATH ?? resolve("config/markets.risex-live.yaml"),
  );
  ...
  const configured = enabled.map((market) => ({
  ...
  const readonly = new RiseXSessionAdapter(marketData, undefined, {
    baseUrl: BASE_URL,
    account,
    markets: configured,
  });
  ...
  const equityGuard = new RiseXSessionEquityGuard(
    ...
    config.accountRisk.dailyLossCapUsd ?? config.accountRisk.sessionLossCapUsd,
    config.accountRisk.weeklyLossCapUsd ?? config.accountRisk.sessionLossCapUsd,
  );
  const equityStatus = equityGuard.status();
  const blockers: string[] = [];
  try {
    assertRiseXPreflight({
      flat: positions.every((position) => position.baseSize === 0),
      openOrderCount: orders.length,
      availableCollateral: balances[0]?.amount ?? 0,
      estimatedInitialMargin,
      marginSafe: !margin.isAtBankruptcyRisk,
    });
  } catch (error) {
    blockers.push(error instanceof Error ? error.message : String(error));
  }
  if (equityStatus.state === "halted")
    blockers.push(equityStatus.haltReason ?? "RISEx equity guard is halted");
  ...
  console.log(
    `Configured resting quotes: ${enabled.reduce((sum, market) => sum + 2 * market.quoteLevels, 0)}`,
  );
  console.log(
  ...
  if (preflightOnly) {
    ...
    console.log(
      "[RISEX LIVE] Read-only preflight complete; no signer was opened and no transaction was submitted.",
    );
    return;
  }
  if (blockers.length) {
    await readonly.disconnect();
    throw new Error("RISEx Live preflight is blocked");
  }
  ...
  if (!privateKey) {
    await readonly.disconnect();
    throw new Error("RISEX_SESSION_SIGNER_PRIVATE_KEY is missing");
  }
  ...
  if (expectedSigner && signer.address.toLowerCase() !== expectedSigner.toLowerCase())
    throw new Error("RISEx session signer address does not match the configured key");
  await readonly.disconnect();
  const execution = new RiseXPermitExecutionTransport(marketData, signer, {
    baseUrl: BASE_URL,
    account,
    markets: configured,
  });
  const adapter = new RiseXSessionAdapter(marketData, execution, {
    baseUrl: BASE_URL,
    account,
    markets: configured,
  });
    ...
    for (const market of enabled) {
      ...
      for (const size of planRiseXFlattenChunks(position, market.riskLimits.maxOrderSize)) {
        ...
        const placed = await adapter.placeOrder({
          ...
          price: position > 0 ? price * 0.995 : price * 1.005,
          size,
          isReduceOnly: true,
        });
        if (!placed.success) {
          failures.push(placed.message);
          break;
        }
        confirmed.push(size);
        await adapter.refreshAccountState();
      ...
      JSON.stringify(
        {
          ...
          finalStatus: flat ? "completed-flat" : "manual-review-required",
        },
        null,
        2,
      ),
