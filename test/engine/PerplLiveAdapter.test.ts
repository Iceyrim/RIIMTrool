...
function readonly(evidence: PerplEquityEvidence) {
  return {
    ...
    getLastAcceptedBlockNumber: vi.fn(() => evidence.blockNumber),
    getSessionEquityEvidence: vi.fn(() => evidence),
    waitForSnapshotAfter: vi.fn(async () => undefined),
...
describe("PerplLiveAdapter", () => {
  it("maps confirmed placement and exact cancellation through the isolated executor", async () => {
    ...
    const adapter = new PerplLiveAdapter(readonly(evidence), executor, undefined, false, {
      BTCUSD: 15,
    });
  ...
  it("waits for state from a newer block before reconciliation", async () => {
    ...
    expect(vi.mocked(source.waitForSnapshotAfter).mock.invocationCallOrder[0]).toBeLessThan(
      vi.mocked(source.refreshAccountState).mock.invocationCallOrder[0]!,
    );
  });

  it("waits from the accepted cursor before enforcing freshness", async () => {
    const evidence: PerplEquityEvidence = {
      balance: "20",
      lockedBalance: "0",
      positionDeposit: "0",
    ...
    const source = readonly(evidence);
    let refreshed = false;
    vi.mocked(source.waitForSnapshotAfter).mockImplementation(async () => {
      refreshed = true;
    });
    vi.mocked(source.refreshAccountState).mockImplementation(async () => {
      if (!refreshed) throw new Error("Perpl on-chain snapshot is stale");
    });
    vi.mocked(source.getSessionEquityEvidence).mockImplementation(() => {
      if (!refreshed) throw new Error("Perpl on-chain snapshot is stale");
      return evidence;
    });

    await expect(
      new PerplLiveAdapter(source, {} as PerplCanaryExecutor).refreshAccountState(),
    ).resolves.toBeUndefined();
    expect(source.getLastAcceptedBlockNumber).toHaveBeenCalledOnce();
  });
  ...
  it("uses the authenticated API stream for immediate order and fill reconciliation", async () => {
    ...
    const api = {
      ...
      getAccountVolume: vi.fn(async () => [
        { market: "ETHUSD", since: "s", until: "u", baseVolume: 0.004, quoteVolume: 9.82204 },
      ]),
    };
    const adapter = new PerplLiveAdapter(
      source,
      {} as PerplCanaryExecutor,
      undefined,
      false,
      {},
      api,
    ...
    expect((await adapter.getAccountVolume({ since: "s", until: "u" }))[0]?.quoteVolume).toBe(
      9.82204,
    );
  ...
  it("refreshes an expired One-Click stream before startup reconciliation", async () => {
    ...
    const api = {
      ...
      getOpenOrders: vi.fn(() => []),
      getPositions: vi.fn(() => []),
      getPositionEvidence: vi.fn(() => ({ blockNumber: 101 })),
      waitForPositionSettled: vi.fn(async () => undefined),
      getOrderFills: vi.fn(async () => []),
      getAccountVolume: vi.fn(async () => []),
    };
    await new PerplLiveAdapter(
      source,
      {} as PerplCanaryExecutor,
      undefined,
      false,
  ...
  it("halts when caught-up on-chain and One-Click position evidence disagree", async () => {
    ...
    const api = {
      ...
      getPositionEvidence: vi.fn((market: string) =>
        market === "ETHUSD"
          ? {
              position: {
                market: "ETHUSD",
  ...
  it("reports the same directionally tick-rounded price sent to One-Click", async () => {
    ...
    const result = await new PerplLiveAdapter(source, executor, undefined, false, {
      BTCUSD: 15,
    }).placeOrder({
      market: "BTCUSD",
      side: "buy",
  ...
  it("clamps normal quotes to the passive book side and permits only reduce-only IOC exits", async () => {
    ...
    const executor = {
      place: vi.fn(async () => ({ state: "confirmed", exchangeOrderId: "api:55" })),
    } as unknown as PerplCanaryExecutor;
    ...
    expect(executor.place).toHaveBeenLastCalledWith(
      expect.objectContaining({ price: 100, postOnly: true }),
    );
    await adapter.placeOrder({
      market: "BTCUSD",
      side: "buy",
