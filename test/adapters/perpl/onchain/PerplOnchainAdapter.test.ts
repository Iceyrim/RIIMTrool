...
class FakeBridge implements PerplBridgeTransport {
  async request(message: BridgeRequest): Promise<BridgeResponse> {
    if (message.command === "hello") return { version: 1, id: message.id, event: "ready", chainId: 10143, exchange: PERPL_TESTNET_EXCHANGE, snapshot: { accountId: 7, blockNumber: "12", blockTimestamp: 1, receivedAt: 1000, positions: [{ symbol: "BTCUSD", baseSize: "0.01", markPrice: "65000", unrealizedPnl: "2", openOrderCount: 1 }], orders: [{ exchangeOrderId: "9", symbol: "BTCUSD", side: "sell", price: "65100", size: "0.01", filledSize: "0", reduceOnly: true }], markets: [{ symbol: "BTCUSD", perpetualId: 16, markPrice: "65000", oraclePrice: "65000", lastPrice: "65000", paused: false, openInterest: "0" }], books: [{ symbol: "BTCUSD", perpetualId: 16, totalOrders: 1 }], eventCount: 0, quiet: true } };
...
describe("PerplOnchainAdapter", () => {
  it("is testnet-only and rejects every mutation", async () => {
    const adapter = new PerplOnchainAdapter(new FakeBridge(), { rpcUrl: "https://testnet-rpc.monad.xyz", markets: [{ symbol: "BTCUSD", perpetualId: 16 }], accountIds: [7] }, () => 1000);
  ...
  it("rejects custom/mainnet and account configuration", () => {
    ...
    expect(() => new PerplOnchainAdapter(new FakeBridge(), { rpcUrl: "https://testnet-rpc.monad.xyz", markets: [{ symbol: "BTCUSD", perpetualId: 16 }] })).toThrow(/account/);
    expect(() => new PerplOnchainAdapter(new FakeBridge(), { rpcUrl: "https://testnet-rpc.monad.xyz", markets: [{ symbol: "BTCUSD", perpetualId: 16 }], accountIds: [0] })).toThrow(/account/);
  });
});
