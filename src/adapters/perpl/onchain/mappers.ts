...
export function validateSnapshot(snapshot: BridgeSnapshot, previousBlock?: bigint): bigint {
  ...
  snapshot.markets.forEach((market) => { if (!((market.symbol === "BTCUSD" && market.perpetualId === 1) || (market.symbol === "ETHUSD" && market.perpetualId === 2))) throw new ExchangeAdapterError("Perpl bridge market identity is invalid"); });
  snapshot.books.forEach((book) => { if (!((book.symbol === "BTCUSD" && book.perpetualId === 1) || (book.symbol === "ETHUSD" && book.perpetualId === 2)) || !Number.isSafeInteger(book.totalOrders) || book.totalOrders < 0) throw new ExchangeAdapterError("Perpl bridge book evidence is invalid"); });
  return block;
}
